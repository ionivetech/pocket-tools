 ...
 *
 * - A file never imported by a unit test is absent from lcov entirely, so the new/modified split
 *   cannot see it. That is now a FAILURE rather than a warning, because the old behaviour was
 *   exactly the hole this gate exists to close: a brand new untested module was printed and then
 *   ignored, and the gate still said PASSED. The rule is a partition, not a threshold, because a
 *   threshold cannot tell "Bun cannot instrument this" from "nobody tested this". Source files
 *   whose extension Bun cannot execute at all (`.vue`, `.md`, `.yml`, `.yaml`, `.json`, `.css`,
 *   `.d.ts` -- declared in `UNINSTRUMENTABLE_EXTENSIONS` and named in this gate's own output
 *   every run, so a reviewer reading a green gate sees what was reported rather than measured)
 *   are reported and never gate; failing on them would be a permanent false red. Every other new
 *   or modified source file must either appear in lcov or carry an entry in
 *   `ABSENT_FROM_LCOV_ALLOWLIST` with a reason, and the gate fails naming anything else. A blank
 *   reason fails as well, so an allowlist whose reasons have stopped being maintained cannot
 *   decay into a silent blanket. Adding an allowlist entry is a reviewed decision, never a way to
...
/** Extensions Bun instruments, so a diff file with one must appear in lcov or be allowlisted. */
const INSTRUMENTABLE_EXTENSIONS: ReadonlySet<string> = new Set([
	".ts",
	".js",
	".mjs",
	".cjs",
...
/**
 * The extensions Bun cannot instrument at all. This is a declared list, not a silent skip: it is
 * named in the gate's own output every run, so a reviewer reading a green gate can see exactly
 * which extensions were reported rather than measured. `.d.ts` is handled in
 * {@link isInstrumentable} because it also ends in `.ts`.
 */
export const UNINSTRUMENTABLE_EXTENSIONS: readonly string[] = [
	".vue",
	".md",
	".yml",
	".yaml",
...
/**
 * Source files that Bun *could* instrument but that no unit test imports, each with the reason
 * it is exempt. An entry is an assertion that something other than unit coverage exercises the
 * file, so the reason is required and a blank one is a failure: an allowlist whose reasons have
 * rotted away is a silent blanket, which is the shape of the hole this gate exists to close.
 * Adding an entry here is a reviewed decision. It is not a way to make a red gate green.
 *
 * @example
 * ```ts
 * ABSENT_FROM_LCOV_ALLOWLIST.get("nuxt.config.ts"); // a reason, never undefined
 * ```
 */
export const ABSENT_FROM_LCOV_ALLOWLIST: ReadonlyMap<string, string> = new Map([
	...
	[
		...
		"generated by `scripts/generate-tool-registry.ts`; the generator itself is unit-tested and " +
			"the registry freshness check fails on drift",
	],
]);
 ...
 * @example
 * ```ts
 * isInstrumentable("app/utils/url-state.ts"); // true  -- must be in lcov
 * isInstrumentable("vue-shims.d.ts"); // false -- declares types, emits no runtime code
 * isInstrumentable("app/components/ToolHost.vue"); // false
 * ```
 */
export function isInstrumentable(path: string): boolean {
	// A `.d.ts` declares types and emits no runtime code, so there is nothing to execute. The
	// check is on the whole path, not on the sliced extension: `vue-shims.d.ts` ends in `.ts`, so
	// taking the last extension would classify it as instrumentable and gate on a file that can
	// never appear in an lcov report.
...
/** Diff files that lcov did not record, bucketed by what the gate does about each one. */
export type AbsentFromLcov = {
	/** Absent, and not something Bun can instrument: reported, never gating. */
	notInstrumentable: string[];
	/** Absent, instrumentable, and allowlisted with a reason: reported, never gating. */
	allowlisted: Array<{ path: string; reason: string }>;
	/** Absent, instrumentable, and not allowlisted: the split cannot see it, so the gate fails. */
	unaccounted: string[];
	/** Allowlisted, but carrying no real reason: the gate fails so the allowlist cannot rot. */
	reasonless: string[];
};

/**
 * Buckets diff files that lcov never instrumented. `allowlist` is a parameter so a test can
 * present a rotting one -- the failure mode this bucket exists to catch cannot be reached through
 * a map that happens to be correct today.
 */
export function partitionAbsentFromLcov(
	paths: readonly string[],
	instrumented: ReadonlySet<string>,
	allowlist: ReadonlyMap<string, string> = ABSENT_FROM_LCOV_ALLOWLIST,
): AbsentFromLcov {
	const absent: AbsentFromLcov = {
		notInstrumentable: [],
		allowlisted: [],
		unaccounted: [],
		reasonless: [],
	...
	for (const path of paths) {
		if (instrumented.has(path)) {
			continue;
		}
		if (!isInstrumentable(path)) {
			absent.notInstrumentable.push(path);
			continue;
		}
		const reason = allowlist.get(path)?.trim();
		if (reason === undefined) {
			absent.unaccounted.push(path);
		} else if (reason === "") {
			absent.reasonless.push(path);
		} else {
			absent.allowlisted.push({ path, reason });
		}
	}
...
export function runGate(inputs: GateInputs, out: GateOutput): number {
	...
	const classification = parseNameStatus(inputs.nameStatus);
	const split = splitByDiff(repoRecords, classification);
	...
	if (absent.notInstrumentable.length > 0) {
		out.log(
			`Coverage gate: ${absent.notInstrumentable.length} source diff files are absent from lcov ` +
				`because Bun cannot instrument ${UNINSTRUMENTABLE_EXTENSIONS.join(", ")}; they are ` +
				`reported and NOT gating: ${absent.notInstrumentable.join(", ")}`,
		);
	}
	for (const each of absent.allowlisted) {
		out.log(`Coverage gate: ${each.path} is absent from lcov but allowlisted: ${each.reason}`);
	}
	for (const path of absent.unaccounted) {
		shortfalls.push(
			`${path} is absent from lcov, so the new/modified split cannot see it and an untested ` +
				"new module would be invisible to this gate (instrument it via a unit test, or " +
				`allowlist it in ABSENT_FROM_LCOV_ALLOWLIST with a reason)`,
		);
	}
	for (const path of absent.reasonless) {
		shortfalls.push(
			`${path} is on the absent-from-lcov allowlist with no reason, so the entry excuses ` +
				"nothing; give it a reason or drop it, before the allowlist becomes a silent blanket",
		);
	}
	acceptable = acceptable && absent.unaccounted.length === 0 && absent.reasonless.length === 0;
